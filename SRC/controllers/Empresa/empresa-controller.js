...
const modificar_empresa = async (req, res, next) => {
    try {
        const { nombres_empresa, direccion_empresa, celular_empresa, correo_empresa } = req.body;
        const { id } = req.params;

        const users = await pool.query('call Editar_Empresa($1,$2,$3,$4,$5)', [nombres_empresa, direccion_empresa, celular_empresa, correo_empresa, id]);
        console.log(users);
...
const imagen_empresa = async (req, res, next) => {
    try {
        ...
        let fd = fs.createReadStream(path.join(__dirname, "../" + users.rows[0].url_foto));
...
const cambiar_foto = async (req, res, next) => {
    try {
        ...
        const users = await pool.query('Call Cambiar_Foto_Empresa($1,$2)', [foto, id_empresa]);
